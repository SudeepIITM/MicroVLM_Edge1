# ==========================================================
# VIDEO QA SYSTEM - CORE LOGIC V6 (V3-STYLE PROVEN LOADING)
# NEW MULTITASK TEMPORAL + SUMMARY-FUSION ARCHITECTURE
# ==========================================================
...
def load_all_models(progress=None):
    """
    V6 hybrid deployment using the proven V3-style model loading pattern.

    Separate Qwen objects are retained:
      - qwen: frozen Qwen3-VL backbone for old Activity/Weapon and
        new People/Location feature extraction.
      - summ_model: separate Qwen3-VL base + summarization PEFT/LoRA.

    Hybrid task mapping remains unchanged:
    ...
    log("1/9 Checking model files...")
    ensure_model_files()

    log("2/9 Loading multitask metadata and label encoders...")
    with open(
        MULTITASK_METADATA_PATH,
        "r",
        encoding="utf-8",
    ...
    log("3/9 Loading old Activity / Weapon classifier bundle...")
    ...
    log(f"4/9 Loading embedding / feature processor: {backbone_id}")
    qwen_processor = AutoProcessor.from_pretrained(
        backbone_id,
        trust_remote_code=True,
    )

    log(f"5/9 Loading embedding / feature Qwen on {DEVICE} (V3-style)...")
    qwen = (
    ...
    log("6/9 Loading summarization processor...")
    summ_processor = AutoProcessor.from_pretrained(
        BASE_MODEL,
        trust_remote_code=True,
    )

    log(f"7/9 Loading separate summarization Qwen + LoRA on {DEVICE} (V3-style)...")
    base_model = (
        Qwen3VLForConditionalGeneration
        .from_pretrained(
            BASE_MODEL,
    ...
    try:
        ...
        summ_model = PeftModel.from_pretrained(
            base_model,
            SUMMARY_CHECKPOINT,
        )
        ...
        log(
            "WARNING: Summary LoRA could not be loaded; "
            f"using summarization base model. Error: {error}"
        )
        summ_model = base_model

    summ_model.eval()

    log("8/9 Loading trained multitask temporal / fusion model...")
    ...
    log("9/9 Finalizing V6 hybrid model bundle...")
    ...
    log(
        "READY: V3-style separate Qwen loading + binary ensemble + "
        "OLD Activity/Weapon + NEW People/Location + summary LoRA."
    )
