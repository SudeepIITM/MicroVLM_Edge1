...
def load_all_models(progress=None):
    """
    Load ONE Qwen3-VL backbone and attach the summarization LoRA to it.

    The same PEFT-wrapped model is shared by:
      - frozen feature extraction (LoRA temporarily disabled)
      - old Activity / Weapon classifiers (LoRA disabled)
      - new People / Location temporal model (LoRA disabled)
      - video summarization (LoRA enabled)

    This avoids loading Qwen3-VL-2B twice on the GPU.
    """
    global _MODELS
    ...
    log(f"4/8 Loading shared processor: {backbone_id}")
    shared_processor = AutoProcessor.from_pretrained(
        backbone_id,
        trust_remote_code=True,
    )

    log(f"5/8 Loading ONE shared Qwen3-VL backbone on {DEVICE}...")
    shared_base_model = (
    ...
    log("8/8 Finalizing shared hybrid model bundle...")

    _MODELS = {
        "metadata": metadata,
        "encoders": encoders,
        # ONE shared model. LoRA is disabled temporarily for embedding paths.
        "qwen": shared_model,
        "qwen_processor": shared_processor,
        "multitask_model": multitask_model,
        "old_mc_bundle": old_mc_bundle,
        # Same shared model with LoRA enabled for summarization.
        "summ_model": shared_model,
        "summ_processor": shared_processor,
    }

    log(
        "READY: one shared Qwen backbone + LoRA summary adapter + "
